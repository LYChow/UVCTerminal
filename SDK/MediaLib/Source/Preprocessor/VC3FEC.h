...
#if !defined(_VC3_FEC_CLASS_H__)
#define _VC3_FEC_CLASS_H__
...
class CVC3FEC : public CSWFECBase 
{
public:
	/*FEC构造数据回调函数指针定义*/
//	typedef int (__stdcall *pPacketCallBack) (const char* pSendData, int nSendDatalen, DWORD dwUser);

	/*FEC构恢复后数据回调函数指针定义*/
//	typedef int (__stdcall *pResumeCallBack) (const char* pSendData, int nSendDatalen, DWORD dwUser, int nChannel, DWORD flag1, DWORD flag2);

	/*FEC帧序号回调函数指针定义*/
//	typedef int (__stdcall *pFrameNoCallBack) (DWORD dwFrameNo, DWORD dwUser);
public:
	/************************************************************************/
	/*FEC包构造相关函数:													*/
	/************************************************************************/
...
private:	
	...
	//恢复数据
	int Resume(const char *pData, int nDatalen, DWORD flag1 = 0, DWORD flag2 = 0);
public:
	CVC3FEC();
	virtual ~CVC3FEC();
private:
	//复制控制
	CVC3FEC(const CVC3FEC &OtherObject);//拷贝构造函数
	CVC3FEC &operator = (const CVC3FEC &OtherObject);//重载赋值
private:
	//通道号
//	int m_nChannel;
...
// 	DWORD m_dwBuildUser;						//回调函数m_BuildCallBack的调用者,如果m_BuildCallBack为全局函数m_dwBuildUser可以为NULL
// 	HANDLE m_hBuidEvent;						//标识 m_BuildCallBack 是否正在执行
// 	DWORD m_dwBuildFlag;						//标识数据源是否发生变化
// 	DWORD m_dwSendTick;							//发送上一个包的时间
// 	pPacketCallBack m_BuildCallBack;			//生成FEC包后回调函数
	
	int m_FECBuildType;				//当前冗余包生成方式
...
// 	DWORD m_dwResumeUser;						//回调函数m_BuildCallBack的调用者,如果m_BuildCallBack为全局函数m_dwBuildUser可以为NULL
// 	BOOL  m_bLostPacket;						//数据包是否连续(不包含冗余包)
// 	HANDLE m_hResumeEvent;						//标识 m_ResumeCallBack 是否正在执行
...
#endif // !defined(_VC3_FEC_CLASS_H__)