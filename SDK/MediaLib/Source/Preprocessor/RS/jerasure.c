...
int jerasure_make_decoding_matrix_(int *matrix, int *erased, int *decoding_matrix, int *dm_ids)
{
  int i, j, tmpmat[K_COUNT*K_COUNT];
  ...
  for (i = 0; i < K_COUNT; i++) {
    if (dm_ids[i] < K_COUNT) {
      for (j = 0; j < K_COUNT; j++) tmpmat[i*K_COUNT+j] = 0;
      tmpmat[i*K_COUNT+dm_ids[i]] = 1;
    } else {
      for (j = 0; j < K_COUNT; j++) {
        tmpmat[i*K_COUNT+j] = matrix[(dm_ids[i]-K_COUNT)*K_COUNT+j];
      }
    }
  }

  i = jerasure_invert_matrix_(tmpmat, decoding_matrix);
  return i;
}
...
int jerasure_matrix_decode_(int *matrix, int row_k_ones, int *erasures,
                          char **data_ptrs, char **coding_ptrs, int size)
{
  int i;
 // int tmpids[K_COUNT];
  int erased[K_COUNT+M_COUNT], decoding_matrix[K_COUNT*K_COUNT], dm_ids[K_COUNT];

  int *perased = jerasure_erasures_to_erased_(erasures, erased);
  if (perased == NULL) return -1;

  //////////////////////////////////////////////////////////////////////

  if (jerasure_make_decoding_matrix_(matrix, erased, decoding_matrix, dm_ids) < 0) 
  {
	  return -1;
  }
  ...
  for (i = 0; i < K_COUNT; i++) 
  {
	  if (erased[i]) 
	  {
		  jerasure_matrix_dotprod_(decoding_matrix+(i*K_COUNT), dm_ids, i, data_ptrs, coding_ptrs, size);
	  }
  }
  //////////////////////////////////////////////////////////////////////
...
/*
   ...
   */
/*
  if (!row_k_ones || erased[K_COUNT]) lastdrive = K_COUNT;

  if (edd > 1 || (edd > 0 && (!row_k_ones || erased[K_COUNT]))) 
  { 
    if (jerasure_make_decoding_matrix_(matrix, erased, decoding_matrix, dm_ids) < 0) 
	{
      return -1;
...
/*
  if (edd > 0) 
  {
    ...
    jerasure_matrix_dotprod_(matrix, tmpids, lastdrive, data_ptrs, coding_ptrs, size);
  }*/
...
/*
  for (i = 0; i < M_COUNT; i++) 
  {
    if (erased[K_COUNT+i]) 
	{
      jerasure_matrix_dotprod_(matrix+(i*K_COUNT), NULL, i+K_COUNT, data_ptrs, coding_ptrs, size);
    }
  }*/
...
int jerasure_matrix_decode__(int k, int m, int w, int *matrix, int row_k_ones, int *erasures,
                          char **data_ptrs, char **coding_ptrs, int size)
{
  int i, edd, lastdrive;
 // int *tmpids;
...
void jerasure_matrix_encode_( int *matrix, char **data_ptrs, char **coding_ptrs, int size)
{
  int i;
  
  for (i = 0; i < M_COUNT; i++) 
  {
	  jerasure_matrix_dotprod_( matrix+(i*K_COUNT), NULL, K_COUNT+i, data_ptrs, coding_ptrs, size);
  }
}
...
int jerasure_invert_matrix_(int *mat, int *inv)
{
  int cols, i, j, k, x, rs2;
  int row_start, tmp, inverse;
  ...
  for (i = 0; i < cols; i++) {
    ...
    if (mat[row_start+i] == 0) { 
      for (j = i+1; j < K_COUNT && mat[cols*j+i] == 0; j++) ;
      if (j == K_COUNT) return -1;
      rs2 = j*cols;
      for (k = 0; k < cols; k++) {
        tmp = mat[row_start+k];
        mat[row_start+k] = mat[rs2+k];
    ...
    tmp = mat[row_start+i];
    if (tmp != 1) {
      inverse = galois_single_divide_(1, tmp);
      for (j = 0; j < cols; j++) { 
        mat[row_start+j] = galois_single_multiply_(mat[row_start+j], inverse);
        inv[row_start+j] = galois_single_multiply_(inv[row_start+j], inverse);
      }
    }
    ...
    for (j = i+1; j != cols; j++) {
      ...
      if (mat[k] != 0) {
        if (mat[k] == 1) {
          ...
          rs2 = cols*j;
          for (x = 0; x < cols; x++) {
            mat[rs2+x] ^= galois_single_multiply_(tmp, mat[row_start+x]);
            inv[rs2+x] ^= galois_single_multiply_(tmp, inv[row_start+x]);
          }
        }
      }
    }
  ...
  for (i = K_COUNT-1; i >= 0; i--) {
    ...
    for (j = 0; j < i; j++) {
      ...
      if (mat[rs2+i] != 0) {
        ...
        mat[rs2+i] = 0; 
        for (k = 0; k < cols; k++) {
          inv[rs2+k] ^= galois_single_multiply_(tmp, inv[row_start+k]);
        }
      }
    }
  }
...
int *jerasure_erasures_to_erased_(int *erasures, int *erased)
{
  int td;
  int t_non_erased;
  int i;
...
void jerasure_matrix_dotprod_(int *matrix_row,
                          int *src_ids, int dest_id,
                          char **data_ptrs, char **coding_ptrs, int size)
{
  int init;
  ...
  dptr = (dest_id < K_COUNT) ? data_ptrs[dest_id] : coding_ptrs[dest_id-K_COUNT];
  ...
  for (i = 0; i < K_COUNT; i++) {
    if (matrix_row[i] == 1) {
      if (src_ids == NULL) {
        sptr = data_ptrs[i];
      } else if (src_ids[i] < K_COUNT) {
        sptr = data_ptrs[src_ids[i]];
      } else {
        sptr = coding_ptrs[src_ids[i]-K_COUNT];
      }
      if (init == 0) {
        memcpy(dptr, sptr, size);
        jerasure_total_memcpy_bytes += size;
                            ...
                            char **data_ptrs, char **coding_ptrs, int size, int packetsize)
{
 ...
 // char *dptr, *pptr;
  ...
  {
//    fprintf(stderr, "jerasure_bitmatrix_encode - packetsize(%d) \% sizeof(int) != 0\n", packetsize);
 //   exit(1);
	  return;
  }
