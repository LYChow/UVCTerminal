...
public:
	...
	int GetTailSize(){return _size-int(_pTail-_pBuffer);}
	template<typename T>
	T& GetHead()
	{
		return *((T*)_pHead);
	...
	{
		return PushData(str,(int)strlen(str)+1);
	}
...
//获取字符串中的一个子字符串
	string 母串,会被改写,
	nIndex 要获取的索引位置
	outArr 如果不为空,则返回从0~nIndex的所有位置的字符串
*/
char* ScanString(char* string,int nIndex,char* outArr[],int* nParsed=NULL,LPCTSTR szSeprator=" \t");
